...
{
    /// <summary>
    /// Interaction logic for AddKunjunganPage.xaml
    /// </summary>
    public partial class AddKunjunganPage : Window
    {
        private AddKunjunganPageViewModel vm;

        public AddKunjunganPage(Kunjungan kunjungan)
        {
            InitializeComponent();
            DataContext = vm = new AddKunjunganPageViewModel(kunjungan);
            if (!string.IsNullOrEmpty(kunjungan.Anggota.Photo))
            {
                imagePhoto.Source = new BitmapImage(new Uri($"{AppDomain.CurrentDomain.BaseDirectory}/Photos/{kunjungan.Anggota.Photo}"));
            }
        }
    ...
    internal class AddKunjunganPageViewModel : ObservableObject
    {
        ...
        public AddKunjunganPageViewModel(Kunjungan kunjungan)
        {
            ...
            Model = kunjungan.Anggota;
            TTL = $"{Model.TempatLahir}, {Model.TanggalLahir.Value.ToString("dd-MM-yyyy")}";
            _ = ShowNewKelas();
